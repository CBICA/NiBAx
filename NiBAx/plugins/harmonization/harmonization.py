...
class Harmonization(QtWidgets.QWidget,BasePlugin):
    ...
    def plotMUSE(self,plotOptions):
        ...
        if 'self.calculated_parameters' not in globals():
    ...
    def DoHarmonization(self):
        ...
        if 'UseForComBatGAMHarmonization' in self.datamodel.data.columns:
            ...
            if not sites_to_harmonize:
                ...
                oos_data = self.datamodel.data[self.datamodel.data['SITE'].isin(sites_to_harmonize)][[x for x in self.datamodel.harmonization_model['ROIs']]].values
                oos_covars = self.datamodel.data[self.datamodel.data.SITE.isin(sites_to_harmonize)][covariates]
